...
const SECRET = process.env.AUTH_SECRET || (() => {
  // Публичный дефолтный секрет удалён: с известным ключом сессии можно подделать
  // (репозиторий открытый). Если AUTH_SECRET не задан — генерируем эфемерный
  // случайный секрет на процесс: сессии не переживут перезапуск, но подделать их
  // нельзя. В продакшене AUTH_SECRET обязателен (задан в Vercel env).
  console.warn("[auth] AUTH_SECRET не задан — использую временный случайный секрет. Задайте AUTH_SECRET в env.");
